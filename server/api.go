...
func init() {
	Endpoints = []*Endpoint{
		...
		// --- Encryption endpoints ---
		{
			...
			CSRFRequired: false,
			NoAuth:       true,
			Handler:      serveUnlockPage,
			Description:  "Serve the encryption unlock page (HTML)",
		},
		{
			Name:         "Unlock API",
			Path:         "/api/unlock",
			...
			CSRFRequired: false,
			NoAuth:       true,
			Handler:      serveUnlockAPI,
			Description:  "Decrypt the private key with a password and unlock encrypted content",
		},
		{
			Name:         "Lock API",
			Path:         "/api/lock",
			Method:       POST,
			CSRFRequired: true,
			Handler:      serveLockAPI,
			Description:  "Re-lock the server, wiping the private key from memory",
		},
		{
			...
			CSRFRequired: false,
			NoAuth:       true,
			Handler:      serveEncryptionStatus,
			Description:  "Return whether encryption is enabled and whether the server is locked",
		},
	}
}
