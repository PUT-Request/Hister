<script lang="ts">
  ...
  let similarityThreshold = $state(
    parseFloat(localStorage.getItem('hister-semantic-threshold') ?? 'NaN') || 0.1,
  );
  ...
  onMount(() => {
    (async () => {
      ...
      if (config.semanticEnabled) {
        ...
        // Also migrate any leftover 0.5 that came from the old hardcoded UI
        // default so the effective threshold matches the server's
        // similarity_threshold (e.g. 0.1).
        const savedThreshold = localStorage.getItem('hister-semantic-threshold');
        if (savedThreshold === null || savedThreshold === '0.5')
